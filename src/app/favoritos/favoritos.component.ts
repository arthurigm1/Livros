  ...
  animations : [
    trigger('fadeInList', [
      transition(':enter', [
        ...
        query('.livro-item', stagger('500ms', [
          animate('1000ms ease-out', style({ opacity: 1 }))
        ]), { optional: true }) // Aparecem com intervalo de 500ms
      ])
    ])
  ]
})
export class FavoritosComponent {
  ...
  constructor(private favoritoService: FavoritoService, private toastService: ToastrService) {}
...
carregarLivrosFavoritos(): void {
  ...
  const favoritos = this.livrosFavoritos.filter(livro => livro.favorito).map(livro => livro.id);
